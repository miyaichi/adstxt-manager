...
describe('AdsTxt Controller Tests', () => {
  ...
  describe('processAdsTxtFile', () => {
    it('should process and parse a valid Ads.txt file', async () => {
      // Arrange
      const parsedRecords = [
        { domain: 'example.com', account_id: 'pub-1', account_type: 'DIRECT', relationship: 'DIRECT', is_valid: true },
        { domain: 'example2.com', account_id: 'pub-2', account_type: 'RESELLER', relationship: 'RESELLER', is_valid: true }
      ];
      
      req.file = {
        buffer: Buffer.from('example.com, pub-1, DIRECT, DIRECT\nexample2.com, pub-2, RESELLER, RESELLER')
  ...
  describe('generateAdsTxtContent', () => {
    it('should generate content for approved records', async () => {
      ...
      const mockRecords = [
        ...
        { ...mockRecord, id: 'record-456', status: 'approved', certification_authority_id: 'abc123' },
      ...
      expect(res.setHeader).toHaveBeenCalledWith('Content-Disposition', 'attachment; filename="ads.txt"');
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.send).toHaveBeenCalledWith(expect.stringContaining('example.com, pub-1234, DIRECT, DIRECT'));
