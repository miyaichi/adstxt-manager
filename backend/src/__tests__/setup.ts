...
jest.mock('../i18n', () => ({
  t: (key: string, options?: any) => {
    ...
    const translations: Record<string, string> = {
      ...
      'email:request.publisher.message': '{{requesterName}} ({{requesterEmail}}) has requested to update your Ads.txt file',
      ...
      'email:request.requester.message': 'Your request to update ads.txt for publisher {{publisherEmail}} has been submitted.',
      'email:request.requester.action': 'You can track the status of your request by clicking the link below:',
      ...
      'email:statusUpdate.message': 'Your ads.txt request has been updated to: <strong>{{status}}</strong>',
