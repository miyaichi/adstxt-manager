...
jest.mock('@aws-amplify/ui-react', () => ({
  ...
  Heading: ({ children, ...props }: any) => <h5 data-testid="heading" {...props}>{children}</h5>
}));
...
describe('RequestItem component', () => {
  test('renders request information correctly', () => {
    ...
    const requesterNameText = textElements.find(el => 
      el.textContent?.includes('Test Requester')
    );
    ...
    expect(detailButton.getAttribute('to')).toBe(`/request/${mockRequest.id}?token=${mockRequest.token}`);
  });
