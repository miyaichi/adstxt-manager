...
jest.mock('@aws-amplify/ui-react', () => {
  ...
  const createAlertComponent =
    () =>
    ({ children, variation, ...props }: any) => (
      <div data-testid={`alert-${variation || 'default'}`} {...props}>
        {children}
      </div>
    );

  const createLoaderComponent = () => (props: any) => <div data-testid="loader" {...props} />;

  const createDividerComponent = () => (props: any) => <hr data-testid="divider" {...props} />;

  const createTabsComponent =
    () =>
    ({ children, currentIndex, onChange, ...props }: any) => {
      ...
      return (
        <div data-testid="tabs" {...props}>
          ...
          <div data-testid="tab-content">{childrenArray[currentIndex]}</div>
        </div>
      );
    };

  const createTabItemComponent =
    () =>
    ({ children, title, ...props }: any) => (
      <div data-testid={`tab-item-${title}`} {...props}>
        {children}
      </div>
    );
  ...
  const createSelectFieldComponent =
    () =>
    ({ label, onChange, options, ...props }: any) => (
...
jest.mock('../../../components/adsTxt/AdsTxtRecordList', () => {
  return jest.fn(({ records, onStatusChange, isEditable }) => (
    <div data-testid="ads-txt-record-list">
      ...
      {isEditable && (
        <button onClick={() => onStatusChange && onStatusChange('test-id', 'approved')}>
          Change Status
        </button>
      )}
    </div>
...
describe('RequestDetail component', () => {
  beforeEach(() => {
    ...
    (adsTxtApi.generateAdsTxtContent as jest.Mock).mockResolvedValue(
      'example.com, account1, DIRECT, id123'
    );
  });
  ...
  test('handles request status update', async () => {
    ...
    await waitFor(() => {
      expect(requestApi.updateRequestStatus).toHaveBeenCalledWith(
        'req-123',
        'approved',
        'test-token'
      );
  ...
  test('handles record status update', async () => {
    ...
    await waitFor(() => {
      expect(adsTxtApi.updateRecordStatus).toHaveBeenCalledWith(
        'test-id',
        'approved',
        'test-token'
      );
